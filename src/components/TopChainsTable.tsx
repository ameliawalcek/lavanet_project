...
import { topChainsDummyData } from "../data/dummyData";
import StickyHeader from "./StickyHeader";
import ChainsRows from "./ChainsTableRows";
import ChainsTableRows from "./ChainsTableRows";
//NOTES------------------------------------
//Make sure to get rid of all the dummy data even in the jsx!!
//-------------------
...
const TopChainsTable: React.FC = () => {
  ...
  return (
    <div>
      <StickyHeader
        topChains={topChainsDummyData}
        blockLimit={BLOCKS_FETCH_LIMIT}
      />
      {isLoading && topChainsDummyData.length === 0 ? (
        <Loader />
      ) : (
        <ChainsTableRows topChains={topChainsDummyData} />
      )}
    </div>
  );
};
